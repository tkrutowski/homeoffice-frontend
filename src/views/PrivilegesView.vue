...
const onUserChange = (event: any) => {
  let {value} = event;
  userStore.getPrivilegesByUserFromDb(value.id)
      .then(privilegeList => privileges.value = privilegeList)
}

const onRowEditSave = (event: any) => {
  let {newData} = event;
  userStore.updatePrivilegeDb(newData)
      .then(() => {
        toast.add({
          severity: "info",
          summary: "Informacja",
        ...
        if (selectedUser.value?.id) {
          userStore.getPrivilegesByUserFromDb(selectedUser.value.id)
              .then(privilegeList => privileges.value = privilegeList)
        }
      })
      .catch(() => {
...
const saveRole = async () => {
  ...
  if (selectedRole.value && selectedUser.value) {
    const result = await userStore.addPrivilegesToUserFromDb(selectedUser.value.id, selectedRole.value.id);
    if (result) {
      toast.add({
        severity: "info",
        summary: "Informacja",
...
const deletePrivilege = async () => {
  if (selectedPrivilege.value && selectedUser.value) {
    const result = await userStore.deletePrivilegesFromUserFromDb(selectedUser.value.id, selectedPrivilege.value.role.id);
    if (result) {
      toast.add({
        severity: "info",
        summary: "Informacja",
...
onMounted(async () => {
  users.value = await userStore.getUsersFromDb();
})
</script>
...
<template>
  ...
  <div class="card flex flex-col justify-center items-center">
    ...
    <Toolbar class="mt-6" style="width: 50rem">
      <template #start>
        <OfficeButton btn-type="office-regular" text="Dodaj" icon="pi pi-plus" class="mr-2" @click="addExistingRole"
                      :disabled="selectedUser === null"/>
        <OfficeButton btn-type="office-save" text="usuń" icon="pi pi-trash" @click="confirmDeletePrivilege"
                      :disabled="!selectedPrivilege"/>
      </template>
    </Toolbar>
    <DataTable v-model:editingRows="editingRows" :value="privileges" editMode="row" dataKey="role.name"
    ...
    >
      ...
      <Column field="role.name" header="Rola" style="width: 30%">
      </Column>
  ...
  <Dialog v-model:visible="addExistingRoleToUserDialog" :style="{ width: '450px' }" header="Dodawanie istniejącej roli"
          :modal="true">
    <div class="flex flex-col gap-6">
      <div>
        <label for="role" class="block small ml-1">Istniejące role</label>
        <Select id="role" v-model="selectedRole" :options="notUserRoles" optionLabel="name" placeholder="Wybierz role"
                fluid></Select>
      </div>
    </div>
    <template #footer>
      <OfficeButton btn-type="office-regular" icon="pi pi-times" @click="hideDialog" text="Anuluj"/>
  ...
  <Dialog v-model:visible="deletePrivilegeDialog" :style="{ width: '450px' }" header="Potwierdzenie" :modal="true">
    <div class="flex items-center gap-4">
      ...
      <span>Czy na pewno chcesz usunąć uprawnienie  <b>{{ selectedPrivilege?.role.name }}</b
      >?</span
      >
    </div>
    <template #footer>
      <OfficeButton btn-type="office-regular" icon="pi pi-times" text="Nie" @click="deletePrivilegeDialog = false"/>
