<script setup lang="ts">
  ...
  const saveRole = async () => {
    ...
    if (selectedRole.value && selectedUser.value) {
      await userStore
        .addPrivilegesToUserFromDb(selectedUser.value.id, selectedRole.value.id)
        .then(() => {
  ...
  const deletePrivilege = async () => {
    if (selectedPrivilege.value && selectedUser.value) {
      await userStore
        .deletePrivilegesFromUserFromDb(selectedUser.value.id, selectedPrivilege.value.role.id)
        .then(() => {
          toast.add({
            severity: 'info',
            summary: 'Informacja',
