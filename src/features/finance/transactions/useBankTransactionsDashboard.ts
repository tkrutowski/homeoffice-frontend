...
} from '@/features/finance/transactions/useBankTransactionFilters';
import type { TransactionCategoryType } from '@/features/finance/transactions/types';
import type {
  BalanceGranularity,
...
export function useBankTransactionsDashboard() {
  ...
  const transactionsQuery = useTransactionsBetweenQuery(dateFrom, dateTo, periodReady);
  const rawTransactions = computed(() => transactionsQuery.data.value ?? []);
  ...
  function buildCategoryBreakdown(type: TransactionCategoryType): CategoryBreakdownItem[] {
    ...
    for (const t of filters.filteredTransactions.value) {
      if (getTransactionCategoryType(t, filters.categories.value) !== type) continue;

      const cat = resolveTransactionCategory(t.transactionCategory, filters.categories.value);
  ...
  function buildChangesByBucket(granularity: ChartGranularity): ChangesBucketData {
    ...
    for (const t of filters.filteredTransactions.value) {
      ...
      const type = getTransactionCategoryType(t, filters.categories.value);
      const amount = parseTransactionAmount(t.amount);
  ...
  async function loadBalanceData() {
    ...
    try {
      ...
      const [balance, series] = await Promise.all([
        fetchCurrentBalance({ dateTo: dateTo.value, ...filterParams }),
        fetchBalanceSeries({
