<script setup lang="ts">
  ...
  import { useDeleteTransactionMutation } from '@/features/finance/transactions/queries/useTransactionsMutations';
  import {
    resolveTransactionCategory,
    resolveTransactionLabelName,
  } from '@/features/finance/transactions/transactionEnrichment';
  ...
  const row = computed(() => props.transaction);
  ...
  const resolvedCategory = computed(() => resolveTransactionCategory(row.value.transactionCategory, categories.value));
  ...
  const displayLabels = computed(() =>
    (row.value.transactionLabel ?? [])
      .map(label => ({
        id: label.id,
        name: resolveTransactionLabelName(label, labels.value),
      }))
  ...
  async function confirmDelete() {
    try {
      await deleteTransactionMutation.mutateAsync(row.value.id);
      toast.add({
        severity: 'success',
        summary: 'Usunięto',
