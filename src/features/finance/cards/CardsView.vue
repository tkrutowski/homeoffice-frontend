<script setup lang="ts">
  ...
  import { useDeleteCardMutation, useUpdateCardMutation } from '@/features/finance/cards/queries/useCardsMutations';
  ...
  const getBank = (idBank: number) => banks.value.find(bank => bank.id === idBank) ?? null;
  ...
  const filteredData = computed((): Card[] => {
    switch (filter.value) {
      case 'INACTIVE':
        return cards.value.filter(card => card.activeStatus === 'INACTIVE');
      case 'ACTIVE':
        return cards.value.filter(card => card.activeStatus === 'ACTIVE');
      case 'ALL':
      default:
        return cards.value;
    }
  ...
  const refreshCards = async () => {
    await Promise.all([banksQuery.refetch(), cardsQuery.refetch()]);
  };
</script>
