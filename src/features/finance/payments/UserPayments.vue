<script setup lang="ts">
  ...
  import { type Installment, type Payment, PaymentStatus } from '@/features/finance/payments/types';
  import { useToast } from 'primevue/usetoast';
  import ConfirmationDialog from '@/components/ConfirmationDialog.vue';
  import ContextMenu from 'primevue/contextmenu';

  import { useUsersStore } from '@/stores/users.ts';
  import { useUpdateLoanStatusMutation } from '@/features/finance/loans/queries/useLoansMutations';
  import { useUpdateFeeStatusMutation } from '@/features/finance/fees/queries/useFeesMutations';
  ...
  const loadingPayments = computed(() => props.loading);
  ...
  const findBankOrFirmName = (payment: Payment) => {
    ...
    if (payment.paymentType === 'LOAN') {
      result = banksQuery.data.value?.find(bank => bank.id === idIssuer)?.name ?? '';
    ...
    if (selectedPaymentTemp.value) {
      ...
      try {
        if (selectedPaymentTemp.value.paymentType === 'LOAN') {
          await updateLoanStatusMutation.mutateAsync({ loanId: selectedPaymentTemp.value.id, status: newStatus });
        } else {
          await updateFeeStatusMutation.mutateAsync({ feeId: selectedPaymentTemp.value.id, status: newStatus });
        }
...
<template>
  <Panel class="mt-5">
    ...
    >
      <template #empty>
        <p v-if="!loadingPayments" class="text-red-500 text-3xl">Nie znaleziono opłat...</p>
      </template>
