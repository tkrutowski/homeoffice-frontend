<script setup lang="ts">
  ...
  import { fetchFee } from '@/features/finance/fees/api/feesApi';
  import { useUpdateFeeInstallmentMutation } from '@/features/finance/fees/queries/useFeesMutations';
  import { useToast } from 'primevue/usetoast';
  import OfficeIconButton from '@/components/OfficeIconButton.vue';
  import type { AxiosError } from 'axios';
  import { PaymentStatus } from '@/features/finance/payments/types';

  const updateFeeInstallmentMutation = useUpdateFeeInstallmentMutation();
  const toast = useToast();
  const loadingFee = ref<boolean>(false);
  ...
  async function savePayment(date: Date, amount: number) {
    ...
    await updateFeeInstallmentMutation
  ...
  const refresh = async () => {
    ...
    try {
      fee.value = await fetchFee(+props.id);
    } finally {
    ...
    <div id="fee-dashboard" class="mx-auto my-3 w-full max-w-[1200px] space-y-4 px-2 sm:px-4">
      <div class="flex w-full min-w-0 items-center gap-2 sm:gap-4">
        ...
        <div class="flex min-w-0 flex-1 items-center justify-center gap-3">
          ...
          <div v-if="loadingFee" class="shrink-0">
