<script setup lang="ts">
  ...
  import { fetchLoan } from '@/features/finance/loans/api/loansApi';
  import { useUpdateLoanInstallmentMutation } from '@/features/finance/loans/queries/useLoansMutations';
  import { useToast } from 'primevue/usetoast';
  import OfficeIconButton from '@/components/OfficeIconButton.vue';
  import type { AxiosError } from 'axios';
  import { PaymentStatus } from '@/features/finance/payments/types';

  const updateLoanInstallmentMutation = useUpdateLoanInstallmentMutation();
  const toast = useToast();
  const loadingLoan = ref<boolean>(false);
  ...
  async function savePayment(date: Date, amount: number) {
    ...
    if (installment.value) {
      ...
      await updateLoanInstallmentMutation
        .mutateAsync(installment.value)
        .then(savedLoan => {
          if (savedLoan) {
  ...
  const refresh = async () => {
    ...
    try {
      loan.value = await fetchLoan(+props.id);
    } finally {
    ...
    <div id="loan-dashboard" class="mx-auto my-3 w-full max-w-[1200px] space-y-4 px-2 sm:px-4">
      ...
      <div class="flex w-full min-w-0 items-center gap-2 sm:gap-4">
        ...
        <div class="flex min-w-0 flex-1 items-center justify-center gap-3">
          ...
          <div v-if="loadingLoan" class="shrink-0">
