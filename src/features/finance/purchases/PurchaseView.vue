<script setup lang="ts">
  ...
  watch(selectedUser, () => {
    ...
    if (selectedUser.value && selectedUser.value?.id > 0) {
      if (isEdit.value) optionCard.value = filterCardsByUser(cards.value, selectedUser.value?.id);
      else optionCard.value = filterCardsByUserAndStatus(cards.value, selectedUser.value?.id, 'ACTIVE');
    }
  });
