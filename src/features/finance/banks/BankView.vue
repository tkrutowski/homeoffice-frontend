<script setup lang="ts">
  ...
  import { useBankQuery } from '@/features/finance/banks/queries/useBanksQueries';
  import { useCreateBankMutation, useUpdateBankMutation } from '@/features/finance/banks/queries/useBanksMutations';
  import { cloneBank } from '@/features/finance/_shared/cloneEntities';
  ...
  const bankId = computed(() => Number(route.params.bankId as string));
  const bankQuery = useBankQuery(bankId, isEdit);
  ...
  const isSaveBtnDisabled = computed(() => {
    return bankQuery.isFetching.value || btnSaveDisabled.value;
  });
  ...
  watch(
    () => bankQuery.data.value,
    data => {
      if (data) {
        bank.value = cloneBank(data);
      }
    }
  );
