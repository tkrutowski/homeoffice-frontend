<script setup lang="ts">
  ...
  const devicesFetchingCount = useIsFetching({ queryKey: deviceKeys.devices.all() });
  const computersFetchingCount = useIsFetching({ queryKey: deviceKeys.computers.all() });
  const allLoading = computed(() => {
    return computersFetchingCount.value > 0 || devicesFetchingCount.value > 0;
  });
