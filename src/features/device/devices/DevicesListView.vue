<script setup lang="ts">
  ...
  const rowsPerPageList = ref(parseInt(localStorage.getItem('rowsPerPageDevicesList') || '20', 10));
  ...
  const firmFilter = computed(() => {
    return [
      ...new Set(devices.value.filter((dev: Device) => dev.firm).map((dev: Device) => dev.firm?.name ?? '')),
    ].sort((a: string, b: string) => (a ?? '').localeCompare(b ?? ''));
  });
  ...
  const handleRowsPerPageChange = (event: DataTablePageEvent) => {
    rowsPerPageList.value = event.rows;
  ...
  const onRowExpand = async (event: { data: Device }) => {
    ...
    try {
      const full = await fetchDevice(id);
      if (full) {
  ...
  <MainPageShell>
    ...
    <Panel class="my-3 mx-2">
      ...
      >
        ...
        <template #empty>
          <h4 v-if="!loadingDevices" class="color-red">Nie znaleziono urządzeń...</h4>
        </template>
