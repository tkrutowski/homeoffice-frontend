<script setup lang="ts">
  ...
  import { cloneDevice } from '@/features/device/_shared/cloneEntities';
  import { useDeviceQuery, useDeviceTypesQuery } from '@/features/device/devices/queries/useDevicesQueries';
  import {
  ...
  const isEdit = ref<boolean>(route.params.isEdit === 'true');
  const routeDeviceId = computed(() => Number(route.params.deviceId as string));

  const { data: deviceData, isFetching: loadingDevice } = useDeviceQuery(
    routeDeviceId,
    computed(() => isEdit.value)
  );
  const { data: typesData, isFetching: loadingDeviceTypes } = useDeviceTypesQuery();
  const devicesTypes = computed(() => typesData.value ?? []);

  const createDeviceMutation = useCreateDeviceMutation();
  const updateDeviceMutation = useUpdateDeviceMutation();
  const createDeviceTypeMutation = useCreateDeviceTypeMutation();
  ...
  const isSaveBtnDisabled = computed(() => {
    return loadingDevice.value || loadingDeviceTypes.value || firmStore.loadingFirms || btnSaveDisabled.value;
  });
  ...
  async function saveDeviceType(name: string) {
    ...
    if (name.length === 0) {
      ...
      await createDeviceTypeMutation
        .mutateAsync({ id: 0, name: name })
        .then(() => {
          toast.add({
            severity: 'success',
            summary: 'Potwierdzenie',
              ...
              <div class="col-start-1 col-span-4">
                ...
                <div class="flex gap-2">
                  ...
                  <OfficeIconButton
                    ...
                    :icon="loadingDeviceTypes ? 'pi pi-spin pi-spinner' : 'pi pi-plus'"
