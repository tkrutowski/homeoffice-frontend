  ...
  <Card class="book_large">
    ...
    <template #content>
      <div class="flex flex-row gap-4">
        ...
        <div class="pt-4">
          <p class="pb-2">
            Tytuł: <span class="text-xl font-semibold pl-2">{{ userbook.book?.title }}</span>
          </p>
          <p class="pb-2">
            Autor: <span class="text-xl font-semibold pl-2">{{ getAuthors }}</span>
          </p>
          <p class="pb-2">
            Autor: <span class="text-xl font-bold">{{ getCategories }}</span>
          </p>
          <p v-if="userbook.book?.series != null" class="pb-2">
            Cykl: <span class="text-xl font-bold">{{ userbook.book.series.title }}</span>
          </p>
          <p v-if="userbook.book?.series != null" class="pb-2">
            Część: <span class="text-xl font-bold">{{ userbook.book.bookInSeriesNo }}</span>
          </p>
          <p class="pb-2">
            Księgarnia: <span class="text-xl font-bold">{{ getBookstoreName }}</span>
          </p>
          <p class="pb-2">
            Stan posiadania:
