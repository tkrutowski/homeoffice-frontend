        ...
        <div class="min-h-0 flex-1 overflow-y-auto">
          <ul v-if="breakdown.length > 0" class="m-0 flex list-none flex-col gap-2 p-0">
            <li
              ...
              class="flex cursor-pointer items-center gap-3 rounded-lg border border-surface-200 bg-surface-50 px-3 py-2 transition-colors hover:border-primary/40 hover:bg-surface-100 dark:border-surface-700 dark:bg-surface-900 dark:hover:border-primary/40 dark:hover:bg-surface-800"
              role="button"
              tabindex="0"
              @click="emit('categoryClick', item)"
              @keydown.enter="emit('categoryClick', item)"
              @keydown.space.prevent="emit('categoryClick', item)"
            >
              <span
                class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg text-sm text-white"
                :style="{ backgroundColor: categoryDisplay(item)?.backgroundColor ?? TRANSACTION_CATEGORY_DEFAULT_COLOR }"
              >
                <i
              ...
              <div class="min-w-0 flex-1">
                ...
                <p class="m-0 text-xs text-surface-600 dark:text-surface-400">
                  {{ item.count }} {{ item.count === 1 ? 'transakcja' : item.count < 5 ? 'transakcje' : 'transakcji' }}
                </p>
              </div>
