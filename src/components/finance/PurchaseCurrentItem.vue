...
const tempPurchase = ref<Purchase | undefined>(
    purchaseStore.getPurchaseById(props.purchase.id)
);
const getFirmName = () => {
...
const submitChangeStatus = async () => {
  ...
  purchaseStore.payForPurchaseDb(props.purchase.id, newStatus)
      .then(() => {
        toast.add({
          severity: "success",
...
<template>
  ...
  <Card
      class="mb-3"
      :class="{'paid': tempPurchase?.paymentStatus.name === 'PAID',
  ...
  >
    ...
    <template #content>
      <div class="purchase-item-row ml-5 mr-5">
        ...
        <div>
          <h3>
            {{ UtilsService.formatCurrency(props.purchase.amount) }} zł
          </h3>
        </div>
      </div>
    </template>
