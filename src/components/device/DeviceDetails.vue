...
const purchaseAmounts = computed(() => {
  return props.devices.map((dev: Device) => dev.purchaseAmount)
  ...
  <Card class="border dark:border-surface-700 border-surface-200 rounded-t-2xl">
    ...
    <template #content>
      ...
      <div v-for="(dev, index) in props.devices" :key="dev.id" class="mb-5">
        <Card>
          <template #title><span class="text-2xl">{{ dev.name }}</span>
            <OfficeIconButton
          ...
          <template #content>
            <div class="flex flex-row w-full">
              <img :src="dev.imageUrl?.length > 0 ? dev.imageUrl :'../../assets/images/no_image.png'" alt="Image"
          ...
          <template #footer>
            <div class="flex justify-end">
              <p class="text-xl">Cena zakupu: <span
                  class="font-bold text-xl">{{ UtilsService.formatCurrency(dev.purchaseAmount) }}</span></p>
            </div>
          </template>
        </Card>
