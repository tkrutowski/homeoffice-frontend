...
<template>
  ...
  >
    <div
      v-if="alerts.length"
      class="max-h-80 overflow-y-auto overflow-x-hidden pr-1"
    >
      <ul class="flex flex-col gap-3">
          ...
          >
            <i
              :class="
                alert.severity === 'warn' ? 'pi pi-exclamation-triangle' : 'pi pi-info-circle'
              "
    ...
    <p v-else class="text-sm text-surface-500 dark:text-surface-400">
      Brak alertów — wszystko wygląda w porządku.
    </p>
  </DeviceDashboardPanel>
</template>
